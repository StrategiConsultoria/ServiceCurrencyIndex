...
def add_indexs(name,serid):
    ...
    index_month_previous = Index.query.filter_by(DATE = date_month_previous, NAME = name).first()
...
def start_schedule(app):
    ...
    sched.add_job(partial(check_indexs,app), 'cron', day_of_week='mon-fri', hour=9, minute=20)
    sched.start()
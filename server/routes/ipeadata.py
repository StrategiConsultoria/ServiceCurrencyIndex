...
def ipeadata_route(file):
    ...
    try:
        ...
        end_year = get_date_in_args(
            'end_year', date_now.year, 1000, date_now.year)
    ...
    indexs = Index.query.filter_by(NAME=index_name).filter(
        Index.DATE.between(start_date, end_date)).order_by(Index.DATE.desc()).all()
    if file == 'json':
        response = {}
        for index in indexs:
        ...
        response = [{'DATE': index.date, 'INDEX': format_money(
            index.index, decimal)} for index in indexs]
